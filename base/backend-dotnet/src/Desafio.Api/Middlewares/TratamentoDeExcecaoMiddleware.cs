...
using Desafio.Api.Extensoes;

namespace Desafio.Api.Middlewares;

public sealed class TratamentoDeExcecaoMiddleware(
    RequestDelegate proximo,
    ILogger<TratamentoDeExcecaoMiddleware> logger)
{
    public async Task InvokeAsync(HttpContext contexto)
    {
        ...
        catch (ExcecaoDeDominio excecao)
        {
            ...
            await EscreverAsync(
                contexto,
                (int)excecao.ParaStatusCode(),
                ErroResponse.De(excecao));
        }
        ...
        ErroResponse resposta)
    {
        ...
        await contexto.Response.WriteAsync(
            JsonSerializer.Serialize(resposta, JsonPadrao.Opcoes));
    }
}
